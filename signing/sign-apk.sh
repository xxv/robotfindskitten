...
#
#   robotfindskitten has a minSdkVersion of 28 (Android 9), which is the first version with the
#   v3 signature scheme and so the first that recognizes a rotated key. The rotation is put in the
#   v3.0 signing block (--rotation-min-sdk-version 28) rather than apksigner's default of the v3.1
#   block, which only Android 13+ reads. So there's no device this can install on that needs a
#   signature from the original key, and the original keystore is never needed here.
#
...
# The app's minSdkVersion (see app/build.gradle.kts), and the version the rotation applies from.
min_sdk=28
...
sign_apk() {
    ...
    # Check the result the way the oldest supported device would see it.
    local report
